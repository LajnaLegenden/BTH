...
Statement : IF LEFT_PARENTHESIS Expression RIGHT_PARENTHESIS Statement
    {
        $$ = new IfStatement("If", "if", @$.begin.line);
        $$->children.push_back($3);  // condition
        $$->children.push_back($5);  // then part
    }
  | IF LEFT_PARENTHESIS Expression RIGHT_PARENTHESIS Statement ELSE Statement
    {
        $$ = new IfStatement("IfElse", "if-else", @$.begin.line);
        $$->children.push_back($3);  // condition
        $$->children.push_back($5);  // then part
        $$->children.push_back($7);  // else part
    }
  | WHILE LEFT_PARENTHESIS Expression RIGHT_PARENTHESIS Statement
    {
        $$ = new WhileStatement("While", "while", @$.begin.line);
        $$->children.push_back($3);  // condition
        $$->children.push_back($5);  // body
    }
  | SYSTEM_OUT_PRINTLN LEFT_PARENTHESIS Expression RIGHT_PARENTHESIS SEMICOLON
    {
        $$ = new PrintStatement("Print", "println", @$.begin.line);
        $$->children.push_back($3);
    }
  | IDENTIFIER ASSIGN Expression SEMICOLON
    {
        $$ = new AssignmentStatement("Assignment", $1, @$.begin.line);
        Node* id = new IdentifierExpression("Identifier", $1, @$.begin.line);
        $$->children.push_back(id);
        $$->children.push_back($3);
    }
  | LEFT_BRACE StatementList RIGHT_BRACE
    {
        $$ = new BlockStatement("Block", "block", @$.begin.line);
...
Expression
  ...
  | Expression ADD Expression {
    $$ = new AddExpression("Operation", "Add", yylineno);
    add_child($$, $1);
    add_child($$, $3);
  }
  | Expression SUBTRACT Expression {
    $$ = new SubExpression("Operation", "Subtract", yylineno);
    add_child($$, $1);
    add_child($$, $3);
  }
  | Expression MULTIPLY Expression {
    $$ = new MulExpression("Operation", "Multiply", yylineno);
    add_child($$, $1);
    add_child($$, $3);
  }
  ...
  | Expression DIVIDE Expression {
    $$ = new DivExpression("Operation", "Devide", yylineno);
    add_child($$, $1);
    add_child($$, $3);
  }
  ...
  | Expression DOT LENGTH {
    $$ = new Node("Statement", "ArrayLength", yylineno);
    add_child($$, $1);
  }
  ...
  | INTEGER_LITERAL {
    $$ = new NumberExpression("Number", $1, yylineno);
  }
  | TRUE {
    $$ = new BooleanExpression("TrueOrFalse", "True", yylineno);
  }
  | FALSE {
    $$ = new BooleanExpression("TrueOrFalse", "False", yylineno);
  }
  | Identifier {
